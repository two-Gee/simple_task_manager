...
router.post('/lists/:id/users', (req, res) => {
    ...
    db.get("SELECT id FROM users WHERE username = ?", [assignedUserName], (err, row) => {
        ...
        db.get("SELECT * FROM listUsers WHERE listId = ? AND userId = ?", [id, assignedUserId], (err, row) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            if (row) {
            ...
            db.run("INSERT INTO listUsers (listId, userId) VALUES (?, ?)", [id, assignedUserId], function(err) {
                if (err) {
                    return res.status(500).json({ error: err.message });
                }
