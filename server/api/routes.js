...
router.post('/lists/:id/users', (req, res) => {
    ...
    db.get("SELECT id FROM users WHERE username = ?", [assignedUserName], (err, row) => {
        ...
        db.get("SELECT * FROM list_users WHERE list_id = ? AND user_id = ?", [id, assignedUserId], (err, row) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            if (row) {
            ...
            db.run("INSERT INTO list_users (list_id, user_id) VALUES (?, ?)", [id, assignedUserId], function(err) {
                if (err) {
                    return res.status(500).json({ error: err.message });
                }
                req.io.to(id).emit('userAssignedToList', { listId: id, assignedUserId });
                res.status(201).json({ listId: id, assignedUserId });
            });
        });
    });
});
